...
func (service *IdGenerateService) initCacheAllSegmentAlloc(ctx context.Context) error {
	if err := service.initCacheSegmentAlloc(ctx, define2.MockBusinessSegmentType, 100); err != nil {
		return err
	}
	return nil
}

func (service *IdGenerateService) initCacheSegmentAlloc(ctx context.Context, segmentType define2.SegmentType, step uint64) error {
	segmentAlloc := define2.NewSegmentAlloc(segmentType, step)
...
func (service *IdGenerateService) preloadSegmentAllocBufferBySegmentType(ctx context.Context, segmentType define2.SegmentType) (err error) {
...
func (service *IdGenerateService) loadSegmentAllocBuffer(ctx context.Context, segmentAlloc *define2.SegmentAlloc) (err error) {
	defer func() {
		r := recover()
		_ = goasync.PanicErrHandler(r)
	}()
	segment := define2.NewSegment(segmentAlloc.Step)
