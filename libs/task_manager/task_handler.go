...
import (
	...
	gopool2 "golib/libs/concurrency/gopool"
	"golib/libs/etcd_helper"
	logger2 "golib/libs/logger"
	"reflect"
	"runtime"
	"sync"
	"time"
...
type TaskHandler struct {
	...
	taskHandlerGoPool gopool2.Pool // 控制handler的并发度
}
...
func NewTaskHandler(name TaskHandlerName, config *TaskConfig, taskFunc interface{}) *TaskHandler {
	...
	return &TaskHandler{
		...
		Config:            config,
		TaskFunc:          taskFunc,
		taskHandlerGoPool: gopool2.NewPool("taskHandlerGoPool", gopool2.NewConfig(gopool2.WithConfigCap(100))),
	}
}
...
func (handler *TaskHandler) handleTask(ctx context.Context, task *Task) {
	ctx = logger2.CtxWithTraceId(ctx, task.TraceId)
	...
	case del:
		...
		_, err := etcd_helper.Context(ctx).
			...
			Commit()
		if err != nil {
			logger2.CtxErrorf(ctx, "delete task fail: %+v", err)
		}
	}
}
...
func (handler *TaskHandler) handlePendingTask(ctx context.Context, task *Task) {
	...
	if err != nil {
		logger2.CtxInfof(ctx, "preemptTask fail: %+v", err)
		return
	}
	// 抢占到了，执行完后需要释放
	defer func() {
		...
		if err != nil {
			logger2.CtxErrorf(ctx, "completeTask fail: %+v", err)
		}
	}()
	// 延迟执行
	...
	if err != nil {
		logger2.CtxErrorf(ctx, "handler.exec err: %s, task:%+v", err.Error(), task)
		return
	}
}
...
func (handler *TaskHandler) completeTask(ctx context.Context, task *Task) error {
	...
	if task.Status == success {
		...
		if err != nil {
			logger2.CtxErrorf(ctx, "completeTask Delete err: %+v", err)
			return err
		}
		logger2.CtxInfof(ctx, "completeTask DeleteResponse.deleted: %d", deleteResponse.Deleted)
	} else {
		putResponse, err := etcd_helper.Put(ctx, task.getTaskKey().String(), task.encode())
		if err != nil {
			logger2.CtxErrorf(ctx, "completeTask Put err: %+v", err)
			return err
		}
		logger2.CtxInfof(ctx, "completeTask PutResponse.header.revision: %d", putResponse.Header.Revision)
	}
	handler.runningTasks.Delete(task.getTaskKey().String())
	return nil
}
...
func (handler *TaskHandler) interruptRunningTask() error {
	handler.runningTasks.Range(func(key, value interface{}) bool {
		...
		ctx := etcd_helper.BindContext(logger2.CtxWithTraceId(context.Background(), task.TraceId))
		task.markPending()
		// 被打断，不算执行次数
		task.ExecCount--
		_, err := etcd_helper.Put(ctx, task.getTaskKey().String(), task.encode())
		if err != nil {
			logger2.CtxErrorf(ctx, "close interruptRunningTask fail: %+v", err)
		}
		// 有错误也不暂停，继续中断其他任务
		return true
	})
